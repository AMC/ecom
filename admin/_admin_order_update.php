...
if ($form_errors == 0) {
  if ($order_status == 'Shipped' && $old_status == 'Pending') {
    include("_email_order_shipped.php");
  }

  if ($db->query("UPDATE cart SET status='$order_status', name='$name', email='$email', phone='$phone', address='$address', address2='$address2', city='$city', state='$state', notes='$notes', shipping='$shipping', discount='$discount', total='$total'  WHERE id='$cart_id' ")) {
